...
struct PerceptionData {
    ...
    // CVT控制器需要的额外字段
    float coolant_temp_celsius = 0.0f;  ///< Coolant temperature (°C)
    float engine_speed_rpm = 0.0f;      ///< Engine speed (RPM) - 别名字段
    bool data_valid = true;             ///< Data validity flag
};

/**
 * @struct PredictionResult
 * @brief Represents prediction result from load predictor.
 */
struct PredictionResult {
    float predicted_load_percent = 0.0f;    ///< Predicted load percentage (0-100%)
    float confidence = 0.0f;                ///< Prediction confidence (0-1)
    uint32_t timestamp = 0;                 ///< Prediction timestamp
    bool is_valid = false;                  ///< Prediction validity flag
};
...
struct CvtConfig {
    ...
    float max_oil_temp = 90.0f;         ///< Maximum oil temperature (°C)
    float min_engine_rpm = 800.0f;      ///< Minimum engine RPM
};
